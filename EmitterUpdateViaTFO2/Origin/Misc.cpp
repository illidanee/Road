...
GLuint CreateAlphaTexture(int w, int h)
{
	...
	float disMax = (float)w;
	for (int i = 0; i < h; ++i)
	{
		for (int j = 0; j < w; ++j)
		{
			float dis = sqrtf((i - w / 2.0f) * (i - w / 2.0f) + (j - h / 2.0f) * (j - h / 2.0f));
			float alpha = pow((disMax - dis) / disMax, 8.0f);
			if (alpha > 1.0f) alpha = 1.0f;
			data[i * w + j] = alpha > 0.0f ? (unsigned char)(alpha * 255) : 0;
		}
	}
...
GLuint CreateTFOProgram(const char* vsShaderPath, const char* gsShadeerPath, GLsizei attribNum, const GLchar* const* attribs, GLenum model)
{
	GLuint program = glCreateProgram();
	...
	char* vsShaderSource = LoadFileContent(vsShaderPath);
	char* gsShaderSource = LoadFileContent(gsShadeerPath);
	glShaderSource(vsShader, 1, &vsShaderSource, 0);
	glShaderSource(gsShader, 1, &gsShaderSource, 0);
	delete[] vsShaderSource;
	delete[] gsShaderSource;
	glAttachShader(program, vsShader);
	glAttachShader(program, gsShader);
	glTransformFeedbackVaryings(program, attribNum, attribs, model);
	glLinkProgram(program);
